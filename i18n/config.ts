...
const resources: Record<string, { common: Translations["common"] }> = {
  en: {
    common: {
      ...
      description:
        `Senior Software Engineer with over ${yearsOfExperience} years of experience in cutting-edge web technology development, specializing in performance optimization, UI/UX, and scalable architecture. Experienced in leading technical teams, ensuring code quality, and mentoring developers to enhance team efficiency.`,
  ...
  vi: {
    common: {
      ...
      description:
        `Kỹ sư phần mềm cấp cao với hơn ${yearsOfExperience} năm kinh nghiệm trong phát triển công nghệ web tiên tiến, chuyên về tối ưu hoá hiệu suất, UI/UX và kiến trúc có khả năng mở rộng. Có kinh nghiệm dẫn dắt nhóm kỹ thuật, đảm bảo chất lượng mã nguồn và hướng dẫn các nhà phát triển để nâng cao hiệu suất làm việc của đội nhóm.`,
  ...
  ja: {
    common: {
      ...
      intro: "クアン・ファムです!",
      description:
        `最先端のWeb技術開発において${yearsOfExperience}年以上の経験を持つシニアソフトウェアエンジニア。パフォーマンス最適化、UI/UX、スケーラブルなアーキテクチャに特化。技術チームのリーダー経験があり、コード品質の確保や開発者の指導を通じて、チームの効率向上に貢献。`,
